...
plot_df['minutes'] = ((plot_df['time'] - t0).dt.total_seconds() / 60).round(1)
...
df_caladan = df_caladan.dropna(subset=['throughput_pps'])
...
ax1.bar(plot_df['minutes'] - offset, plot_df['throughput_Mbps_sm'],
        width=single_bar_w, label='Tangle', color='tab:blue')
ax1.bar(plot_df['minutes'] + offset, df_caladan['throughput_Mbps_sm'],
        width=single_bar_w, label='Caladan', color='tab:red')
...
fig.tight_layout()
plt.savefig("throughput_tangle_caladan_combined_mbps_pps.pdf")
plt.show()
