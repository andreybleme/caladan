...
# ============================
# Paste your data below (same header) for Dataset A
# ============================
# TANGLE_DATA = """Distribution, Target, Actual, Dropped, Never Sent, Median, 90th, 99th, 99.9th, 99.99th, Start, StartTsc
# ..."""
...
# ============================
# Paste your data below (same header) for Dataset B (leave empty to plot only A)
# ============================
CALADAN_DATA = """
...
# Helpers
# ----------------------------
def normalize_and_read(data_text: str) -> pd.DataFrame:
    """Normalize header (remove extra spaces) and read CSV from the pasted block."""
...
def build_cdf(df: pd.DataFrame):
    """Combine all percentiles and return sorted latency and CDF."""
...
# Read inputs
# ----------------------------
df_a = normalize_and_read(TANGLE_DATA)
df_b = normalize_and_read(CALADAN_DATA)

if df_a is None and df_b is None:
    raise ValueError("Please provide at least one dataset in TANGLE_DATA or CALADAN_DATA.")
...
if df_a is not None:
    x_a, cdf_a = build_cdf(df_a)
    # Optional: start at (0,0)
    # x_a = np.insert(x_a, 0, 0)
    # cdf_a = np.insert(cdf_a, 0, 0)
    plt.plot(x_a, cdf_a, linewidth=2, linestyle='--', label="Tangle", color="steelblue")
    plotted_any = True

if df_b is not None:
    x_b, cdf_b = build_cdf(df_b)
    # Optional: start at (0,0)
    # x_b = np.insert(x_b, 0, 0)
    # cdf_b = np.insert(cdf_b, 0, 0)
    plt.plot(x_b, cdf_b, linewidth=2, label="Caladan", color="red")
    plotted_any = True

if not plotted_any:
    raise RuntimeError("No data plotted. Check TANGLE_DATA/CALADAN_DATA contents.")
...
# Add "better" annotation (upper-left is better latency)
plt.annotate(
    "better",
    xy=(min(x_a.min() if df_a is not None else x_b.min(),
            x_b.min() if df_b is not None else x_a.min()), 0.95),  # arrow tip (near top-left)
    xytext=(80, 0.7),  # text position (adjust if needed)
...
plt.savefig("tanglevscaladan_latency_cdf.pdf", dpi=160)
plt.show()
