...
{
    log_debug("tx: tx_pktmbuf_completion_pool_create started");
    ...
    log_debug("tx: RTE_ALIGN ok");
	elt_size = (sizeof(struct rte_mbuf) + (unsigned)priv_size);
	mbp_priv.mbuf_data_room_size = 0;
	mbp_priv.mbuf_priv_size = priv_size;
    log_debug("tx: mbp_priv.mbuf_priv_size = priv_size");

	log_debug("tx: mp before");
	mp = rte_mempool_create_empty(name, n, elt_size, 0,
		 sizeof(struct rte_pktmbuf_pool_private), socket_id, 0);
	if (mp == NULL)
		return NULL;
    log_debug("tx: rte_mempool_create_empty");
    ...
    log_debug("tx: rte_mempool_set_ops_byname");
	rte_pktmbuf_pool_init(mp, &mbp_priv);
    log_debug("tx: rte_pktmbuf_pool_init"); // Segmentation fault after this log
	...
	// 	return NULL;
	// }
    // log_debug("tx: rte_mempool_populate_default");

	rte_mempool_obj_iter(mp, rte_pktmbuf_init, NULL);
    log_debug("tx: rte_mempool_obj_iter 1");
	rte_mempool_obj_iter(mp, tx_pktmbuf_priv_init, NULL);
    log_debug("tx: rte_mempool_obj_iter 2");
...
int tx_init(void)
{
	...
	tx_mbuf_pool = tx_pktmbuf_completion_pool_create("TX_MBUF_POOL_B",
